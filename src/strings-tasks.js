...
function orderAlphabetically(str) {
  const strArr = [...str];
  return strArr.sort((a, b) => a.localeCompare(b)).join('');
}
 ...
 *
 * @param {string} formattedStr - The input string.
...
function isPalindrome(str) {
  const symbols = [',', ' ', '!', '?'];
  let formattedStr = str.toLocaleLowerCase();

  symbols.forEach((s) => {
    formattedStr = formattedStr.replaceAll(s, '');
  });

  for (let i = 0; i < formattedStr.length / 2; i += 1) {
...
function encodeToRot13(str) {
  ...
  [...str].forEach((char) => {
    ...
    if (charIndex === -1) {
      result += char;
    } else if (char === alphabet[charIndex]) {
      result += alphabet[(charIndex + 13) % 26];
    } else {
      result += alphabet[(charIndex + 13) % 26].toLowerCase();
    }
  });
  return result;
}
